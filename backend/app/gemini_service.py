...
class GeminiService:
    def __init__(self) -> None:
        ...
        self.model = (os.getenv("GEMINI_MODEL", "").strip() or "gemini-3.5-flash").strip()
    ...
    def _send_prompt(self, prompt: str, *, use_search: bool = False, temperature: float = 0.2) -> Dict[str, Any]:
        ...
        try:
            ...
            if "429" in message or "RESOURCE_EXHAUSTED" in message or "quota" in message.lower():
                raise GeminiQuotaError("Gemini API quota exceeded. Please wait a bit or upgrade your API plan.") from exc
            if "401" in message or "403" in message or "API key" in message.lower():
                raise GeminiServiceError("Gemini API authentication failed. Check your API key.") from exc
            raise GeminiServiceError(f"Gemini API request failed: {message}") from exc

        text = self._extract_text(response)
        return self._parse_json_payload(text)

    def analyze_profile(self, profile: StudentProfile) -> CareerAnalysis:
        prompt = f"""
...
- The JSON object must have exactly these keys: summary, readiness, strengths, skill_gaps, priority_skills, next_action, reason.
Profile:
- Name: {profile.name}
...
"""
    ...
    def find_opportunities(self, profile: StudentProfile, category: str = "all") -> List[Opportunity]:
        prompt = f"""
...
Each opportunity must include: title, category, organizer, description, eligibility, location, mode, deadline, status, url, source, match_score, why_it_matches.
Rules:
...
- Work type: {profile.work_type}
"""
        payload = self._send_prompt(prompt, use_search=True)
        opportunities_payload = payload.get("opportunities", []) if isinstance(payload, dict) else []
        ...
        for item in opportunities_payload:
            ...
            try:
                ...
                continue
            key = (opportunity.url.strip().lower(), opportunity.title.strip().lower())
            if key in seen:
                continue
            seen.add(key)
    ...
    def generate_projects(self, profile: StudentProfile, analysis: CareerAnalysis) -> List[Project]:
        prompt = f"""
...
Each project must include: title, category, difficulty, estimated_weeks, required_skills, skills_gained, description, problem_statement, tech_stack, career_value, why_it_matches.
Rules:
- Create 8 diverse projects tailored to the student's profile.
- Base them on the target role, dream company, current skills, skill gaps, existing projects, hackathons, internships, weekly hours, and target weeks.
...
Priority skills: {', '.join(analysis.priority_skills)}
"""
        payload = self._send_prompt(prompt)
        ...
        for item in projects_payload:
            if not isinstance(item, dict):
                continue
            try:
                validated.append(Project.model_validate(item))
            except Exception:
                continue
        return validated

    def generate_roadmap(self, profile: StudentProfile, analysis: CareerAnalysis) -> List[RoadmapItem]:
        weeks = self._parse_weeks(profile.target_weeks)
        prompt = f"""
You are CareerPilot designing a personalized roadmap.
Return strict JSON only with an object containing the key 'roadmap'.
Each roadmap item must include: week, title, goal, tasks, deliverable.
Rules:
- Create exactly {weeks} stages because the student targets {profile.target_weeks}.
- Base the roadmap on target role, skill gaps, current skills, existing experience, weekly hours, and target weeks.
...
"""
        payload = self._send_prompt(prompt)
        roadmap_payload = payload.get("roadmap", []) if isinstance(payload, dict) else []
        validated: List[RoadmapItem] = []
        for item in roadmap_payload:
            if not isinstance(item, dict):
                continue
            try:
                validated.append(RoadmapItem.model_validate(item))
            except Exception:
                continue
        return validated

    def _parse_weeks(self, target_weeks: str) -> int:
        match = re.search(r"(\d+)", target_weeks or "")
